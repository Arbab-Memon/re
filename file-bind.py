#Coded By Hanief Aktivisme\Tokoh Masyarakat
#YouTube Hanief Aktivis
#Facebook m.facebook.com/hanief.aktivisme.86
import os,sys,time

def main():
    time.sleep(1)
    os.system ('clear')
    print '[96m'
    os.system ('figlet HACK FACEBOOK 2020')
    print'[92m================================='
    print'[97m Author : [96mHanief Aktivisme '
    print'[97m Team   : [96mFreedom (Kebebasan) '
    print'[92m================================='
    print'[92m++++++++++++ [97mM E N U [92m++++++++++++'
    print'[92m[[97m1[92m] [97mHack Fb v1.7 '
    print'[92m[[97m2][92m [97mHack Fb v1.8 '
    print'[92m[[97m3[92m] [97mHack Fb Premium '
    print'[92m[[97m4[92m] [97mHack Fb New'
    print'[92m[[97m5[92m] [97mHack Fb iDiamond '
    print'[92m[[91m0[92m] [91mExit '
    gans = raw_input ('[97m==>[93m ')
    if gans in ['1']:
        time.sleep(1)
        os.system ('git clone https://github.com/Mr-XsZ/Dark-Fb')
        print ('[97m$ lanjut ketikan [93mcd Dark-Fb')
        print ('[97m$ lalu [93mpython2 dark.py')
        exit()
    if gans in ['2']:
        time.sleep(1)
        os.system ('git clone https://github.com/wira2611/Pro2611')
        print ('[97m$ lanjut ketikan [93mcd Pro2611')
        print ('[97m$ lalu[93m python2 Pro.py')
        exit()
    if gans in ['3']:
        time.sleep(1)
        os.system ('git clone https://github.com/TheMagizz/DarkPremium')
        print ('[97m$lanjut ketikan [93mcd DarkPremium')
        print ('[97m$ lalu [93mpython2 DarkFB.py')
        exit()
    if gans in ['4']:
        time.sleep(1)
        os.system ('git clone https://github.com/KingMrZ17/DarkFbNew')
        print ('[97m$ lanjut ketikan [93mcd DarkFbNew')
        print ('[97m$ lalu [93mpython2 darkfb.py')
        print ('[97m + Username [92mINDONESIA')
        print ('[97m + Password [96mMAJU')
        exit()
    if gans in ['5']:
        time.sleep(1)
        os.system('git clone https://github.com/wira2611/diamon')
        print ('[97m$ lanjut ketikan[93m cd diamon')
        print ('[97m$ lalu [93m python2 diamond-1.py')
        exit()
    if gans in ['0']:
        time.sleep(2)
        exit()
    else:
        time.sleep(1)
        print '[97m Pilih Yg Benar [92mGoblok . . .'
        time.sleep(1)
        main()

main()
